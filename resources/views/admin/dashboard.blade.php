...
@section('content')
    ...
    <div class="row g-4 mt-0">
        @isset($recentPackageEnquiries)
            <div class="col-xl-6"><x-admin.dashboard-table title="Recent package enquiries" :url="route('admin.package-enquiries.index')"><thead><tr><th>Customer</th><th>Package</th><th>Status</th><th>Date</th></tr></thead><tbody>@forelse($recentPackageEnquiries as $enquiry)<tr><td><a href="{{ route('admin.package-enquiries.show', $enquiry) }}">{{ $enquiry->customer_name }}</a><small>{{ $enquiry->email }}</small></td><td>{{ $enquiry->package->title }}</td><td><span class="badge text-bg-{{ $enquiry->status === 'new' ? 'warning' : 'secondary' }}">{{ str($enquiry->status)->replace('_', ' ')->title() }}</span></td><td>{{ $enquiry->created_at->format('d M') }}</td></tr>@empty<tr><td colspan="4" class="text-center text-secondary py-4">No package enquiries yet.</td></tr>@endforelse</tbody></x-admin.dashboard-table></div>
        @endisset
